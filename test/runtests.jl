...
@testset "LabwareBuilding" begin
    for container in container_db 
        lw =Labware(named_id(container.name),container)
...
@testset "StockMath" begin 
    ...
    s2=Stock(iron_nitrate,20u"g",Labware(con50),1)
    s3=Stock(cdm_aas,40u"ml",Labware(con50),1)

    s1,s4=transfer(s1,s2,10u"ml")
    @test typeof(s1)==LiquidStock
    @test typeof(s2)==SolidStock
    @test typeof(s3)==LiquidStock
    @test s4.quantity==10u"ml"
    @test s4.composition.ingredients[ingredient_db[findfirst(x->x.name=="iron_nitrate",ingredient_db)]]==2000u"g/l"
    @test (s4.labware,s4.position)==(s2.labware,s2.position)
    @test s1.quantity==40u"ml"
