    ...
    function Solution(ingredients)  
        ...
        for solid_ingredient in solids 
            ...
            elseif isa(ingredients[solid_ingredient],Unitful.Molarity)
                ingredients[solid_ingredient]=convert(u"g/L",ingredients[solid_ingredient],solid_ingredient)
            else 
...
function Base.show(io::IO,s::Solution)
    printstyled(io, "Solution ($(length(collect(keys(s.ingredients)))) ingredient(s))\n";bold=true)
