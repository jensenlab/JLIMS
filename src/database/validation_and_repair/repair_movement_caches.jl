function repair_movement_caches(ledger_id::Integer)
    ...
    for (prt,child) in participants
        ...
        for cache in eachrow(caches)
            ...
            if cache_sed_id  == sequence_id || children(old_loc) != children(new_loc) # cache has been invalidated --replace the cache 
        ...
        for cache in eachrow(caches)
            ...
            if cache_seq_id == sequence_id || JLIMS.parent(old_loc) != JLIMS.parent(new_loc) # cache has been invalidated --replace the cache 
                cache_ledger_id=get_last_ledger_id(cache_seq_id)
