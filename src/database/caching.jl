function cache(loc::Location,ledger_id=get_last_ledger_id())
    ...
    children_id=cache(children(loc))
    ...
    execute_db("INSERT INTO Caches(LocationID,CacheSetID,LedgerID) Values($(location_id(loc)),$(id),$(ledger_id))")
    return nothing
end 
...
function cache(a::AttributeDict)
    ...
    if isnothing(a)
        ...
        for attr in attrs 
            ...
            execute_db("INSERT OR IGNORE INTO CachedAttributes(AttributeSetID,AttributeID,Value,Unit) Values($id,$(typeof(attr)),$(ustrip(val)),$(string(unit(val))))")
        end 
    end 
    return id 
end 
...
if isnothing(id)
    ...
    for col in 1:cols
        for row in 1:rows
            execute_db("INSERT OR IGNORE INTO CachedChildren(ChildSetID,ChildID,RowIdx,ColIdx) Values($id,$(location_id(c[row,col])), $row,$col)")
        end
    end 
end
return id 
